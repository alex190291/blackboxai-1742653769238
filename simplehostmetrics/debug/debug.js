...
class DebugTools {
        ...
        styleSheets.forEach((sheet, sheetIndex) => {
            try {
                ...
                if (sheet.href && (new URL(sheet.href)).origin !== window.location.origin) {
                    // Create a specific message for cross-origin sheets
                    this.log(`Skipping cross-origin stylesheet: ${sheet.href}`, 'info');
                    return;
                }
                ...
                // If there's a CORS error, fetch the stylesheet directly
                if (sheet.href && !sheet.href.startsWith('data:')) {
                    this.log(`Attempting to fetch stylesheet: ${sheet.href}`, 'info');
                    this.fetchAndParseStylesheet(sheet.href, sheetIndex);
                } else {
                    this.log(`Cannot access rules in stylesheet ${sheetIndex}: ${e.message}`, 'warn');
                }
            }
        });
    ...
    /**
     * Fetch and parse a stylesheet that couldn't be accessed directly
     * @param {string} href - The URL of the stylesheet
     * @param {number} sheetIndex - The index of the stylesheet
     */
    fetchAndParseStylesheet(href, sheetIndex) {
        fetch(href)
            .then(response => {
                if (!response.ok) {
            ...
            .then(cssText => {
                // Create a temporary style element to parse the CSS
                ...
                this.log(`Successfully parsed stylesheet: ${href}`, 'info');
            })
            .catch(error => {
                this.log(`Error fetching stylesheet ${href}: ${error.message}`, 'error');
            });
    }
