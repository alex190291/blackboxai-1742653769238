...
from sqlalchemy import Column, Integer, Float, String, BigInteger, DateTime, ForeignKey  # SQLAlchemy column types.
import datetime
...
class User(db.Model, UserMixin):
    ...
    # Relationship to track sessions
...
class UserSession(db.Model):
    """
    UserSession Model:
    Represents an active user session with information about the device, IP, and login time.
    Used for tracking user logins and allowing session termination.
    """
    __tablename__ = 'user_session'
    id = Column(Integer, primary_key=True)           # Primary key for the session
    user_id = Column(Integer, ForeignKey('user.id'), nullable=False)  # Foreign key to the user
    ...
    user_agent = Column(String(255), nullable=True)   # User agent (browser/device)
    login_time = Column(DateTime, default=datetime.datetime.utcnow)  # Time of session start
    last_active = Column(DateTime, default=datetime.datetime.utcnow)  # Last activity time
