...
function checkAllUpdates() {
  ...
  btn.disabled = true;
  btn.innerHTML = `↻ ${window.t('Check for updates')} <div class="spinner"></div>`;
  fetch("/check_all", { method: "POST" })
    ...
    .then((d) => {
      if (d.status !== "already_in_progress") {
        ...
        btn.innerHTML = `↻ ${window.t('Check for updates')}`;
        btn.disabled = false;
      }
    })
    .catch((err) => {
...
function pollCheckAllStatus() {
  ...
  fetch("/check_all_status")
    ...
    .then((st) => {
      if (st.in_progress) {
        btn.innerHTML = `${window.t('Checking')}... (${st.checked}/${st.total})`;
      } else {
        btn.innerHTML = `↻ ${window.t('Check for updates')}`;
        btn.disabled = false;
